...
## 〇、修订纪要（与仓库及 UI 先行方案对齐）

- **UI**：以 **HeroUI v3**（`@heroui/react` / `@heroui/styles`）为准；Framer Motion 为可选，动效阶段按需引入。
- **路由**：**文档编辑主路径为 `/work/[documentId]`**（与划水 AI 一致）。本方案 §3.1 中曾写的 `/doc/[docId]` 若需保留，仅作为 **`next.config` rewrite 或 301 别名** 指向 `/work/...`，避免双路由长期并存。
- **Layout**：营销站与工作台通过 **Route Groups** 拆分：`(marketing)` 保留 Navbar/Footer；`(workspace)` 为全屏 `AppShell`（见仓库 `slack-frontend/app/(workspace)/`）。
- **工作台主题**：默认浅色；与营销站深色默认可通过子 layout 覆盖 `ThemeProvider`。
- **与 UI 先行规划的关系**：详见同目录 [划水ai_ui复刻规划_68e19b18.plan.md](./划水ai_ui复刻规划_68e19b18.plan.md)。**Phase 0** 仅交付 UI Shell + mock，不引入 Prisma、Tiptap、Yjs、业务 API；**Phase 1** 起按下文各 Phase 推进后端与编辑器。

### Phase 0：工作台 UI 先行（可选冲刺）

1. Route Groups + `/work/[id]` 页面骨架  
2. 侧栏、首页、Modal、文档页三栏 mock（静态数据）  
3. 视觉走查对齐划水工作台
...
- **UI 库**: HeroUI 3.x（`@heroui/react`）+ Tailwind CSS 4；Framer Motion 按需（动效阶段可选）
...
│
├── work/
│   └── [documentId]/page.tsx         # 文档编辑页（核心：编辑器+AI+协同；主路径，对标划水）
│
├── doc/                              # 可选：仅作别名时保留（rewrite → /work/...）
│   └── [docId]/page.tsx
│
├── template/
│   ├── page.tsx                      # 模板市场
...
**文档编辑页 (`/work/[documentId]`，主路径；`/doc/[docId]` 若存在则应为别名)**:
