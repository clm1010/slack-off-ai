---
name: 摸鱼AI完整开发方案
overview: 基于 Next.js 16 + React 19 + Tailwind 4 + Tiptap 技术栈，参照划水AI（HuaShui AI）100% 实现全部功能，涵盖用户系统、文档管理、富文本编辑器、AI写作/处理/聊天、多人协同编辑、模板、分享权限、PDF导出、支付订阅、全链路部署等。
todos:
  - id: phase1-monorepo
...
```
slack-off-ai/
├── slack-frontend/           # Next.js 16 全栈应用（前端 + API Routes）
│   ├── pages/                # 页面 + API 路由
│   ├── components/           # UI 组件
│   ├── hooks/                # 自定义 Hooks
│   ├── lib/                  # 工具库（prisma client, auth, ai, storage）
...
```
pages/
├── _app.tsx                          # 全局 Provider（Auth/Theme/i18n/Zustand）
├── _document.tsx                     # HTML 文档
├── index.tsx                         # 产品落地页
│
├── auth/
│   ├── login.tsx                     # 登录（邮箱+密码 / OAuth）
│   ├── register.tsx                  # 注册
│   └── forgot-password.tsx           # 忘记密码
│
├── dashboard/
│   └── index.tsx                     # 工作台（最近文档/收藏/快速新建）
│
├── workspace/
│   └── [workspaceId]/
│       └── index.tsx                 # 工作空间文档树视图
...
│
├── chat/
│   └── index.tsx                     # AI 聊天助手（独立页面）
│
├── share/
│   └── [token].tsx                   # 公开分享页（无需登录）
│
├── settings/
│   ├── index.tsx                     # 个人设置（头像/名称/密码/语言）
│   ├── workspace.tsx                 # 工作空间设置（成员/角色）
│   └── billing.tsx                   # 订阅与计费
│
├── pricing/
│   └── index.tsx                     # 定价页面
│
└── api/
    ├── auth/[...nextauth].ts         # NextAuth 认证
    ├── documents/
    │   ├── index.ts                  # GET 列表 / POST 创建
    │   ├── [id].ts                   # GET/PATCH/DELETE 单个文档
    │   ├── [id]/duplicate.ts         # POST 复制
    │   ├── [id]/move.ts              # POST 移动
    │   ├── [id]/restore.ts           # POST 恢复
    │   ├── [id]/permanent.ts         # DELETE 永久删除
    │   ├── [id]/versions.ts          # GET 版本列表 / POST 创建版本快照
    │   ├── [id]/versions/[vId].ts    # GET 版本详情 / POST 恢复到此版本
    │   ├── [id]/share.ts             # POST 生成/更新分享 / DELETE 取消分享
    │   ├── [id]/favorite.ts          # POST 收藏 / DELETE 取消收藏
    │   ├── trash.ts                  # GET 回收站列表
    │   └── search.ts                 # GET 全文搜索
    ├── templates/
    │   ├── index.ts                  # GET 列表 / POST 创建
    ...
    ├── ai/
    │   ├── writing.ts                # POST AI 写作（流式）
    │   └── chat.ts                   # POST AI 聊天（流式）
    ├── chat/
    │   ├── sessions.ts               # GET 会话列表 / POST 创建
    │   └── [sessionId]/messages.ts   # GET 消息历史
    ├── upload/
    │   ├── image.ts                  # POST 图片上传 -> OSS
    │   └── cover.ts                  # POST 封面图上传
    ├── share/
    │   └── [token].ts                # GET 分享文档（公开）
    ├── payment/
    │   ├── plans.ts                  # GET 套餐列表
    │   ├── create-order.ts           # POST 创建订单
    │   ├── alipay-notify.ts          # POST 支付宝异步回调
    │   └── alipay-return.ts          # GET 支付宝同步回调
    ├── user/
    │   ├── profile.ts                # GET/PATCH 用户资料
    │   └── quota.ts                  # GET AI 配额使用情况
    └── workspace/
        ├── index.ts                  # GET/POST 工作空间
        └── [id]/members.ts           # GET/POST/DELETE 成员管理
```
...
export default async function handler(req, res) {
  const { action, content, context, options } = req.body;

  // 检查用户配额
  const quota = await checkUserQuota(userId);
  if (quota.exceeded) return res.status(429).json({ error: 'Quota exceeded' });
...
export async function withDocumentAccess(req, res, handler) {
  // 检查用户是否有权访问该文档（workspace membership）
}
```
...
- 方案一（推荐）：后端使用 Puppeteer 渲染（高保真）
  - API Route `/api/documents/[id]/export-pdf`
...
// pages/api/payment/alipay-notify.ts
// 1. 验签
// 2. 更新 Order 状态为 paid
...
export default async function handler(req, res) {
  // 1. 解析 multipart form data (formidable/multer)
  // 2. 生成唯一文件名: `images/${userId}/${timestamp}-${filename}`
  // 3. 上传到 OSS: client.put(key, fileBuffer)
  // 4. 返回 CDN URL: `https://${bucket}.${region}.aliyuncs.com/${key}`
}
```
...
# ===== 文件上传 =====
ali-oss
formidable  # API Route 解析 multipart
